...
{
	DelayPluginAudioProcessor* thisProcessor = getProcessor(); // set processor
}
...
void DelayPluginAudioProcessorEditor::sliderValueChanged(Slider* slider) 
{
	...
	if (slider == &delayTimeSlider) {
		getProcessor()->setParameterNotifyingHost(DelayPluginAudioProcessor::delayTimeParam, (float)delayTimeSlider.getValue());
	}
	if (slider == &feedbackSlider) {
		getProcessor()->setParameterNotifyingHost(DelayPluginAudioProcessor::feedbackParam, (float)feedbackSlider.getValue());
	}
	if (slider == &dryMixSlider) {
		getProcessor()->setParameterNotifyingHost(DelayPluginAudioProcessor::dryMixParam, (float)dryMixSlider.getValue());
	}
	if (slider == &wetMixSlider) {
		getProcessor()->setParameterNotifyingHost(DelayPluginAudioProcessor::wetMixParam, (float)wetMixSlider.getValue());
	}
}
