...
float DelayPluginAudioProcessor::getParameter(int index)
{
...
void DelayPluginAudioProcessor::setParameter(int index, float newValue) 
{
...
{
	// Create XML
	XmlElement xml("delayPluginSettings");
...
void DelayPluginAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
	...
	// Retrieve settings
	if (xmlState != nullptr)
		if (xmlState->hasTagName("delayPluginSettings"))
		{
