	...
	: AudioProcessorEditor(&p), processor(p)
{
	...
	getLookAndFeel().setColour(Slider::thumbColourId, Colours::yellow);
...
{
	PingPongDelayAudioProcessor* thisProcessor = getProcessor(); // set processor
}
...
void PingPongDelayAudioProcessorEditor::sliderValueChanged(Slider* slider)
{
	...
	if (slider == &delayTimeSlider) {
		getProcessor()->setParameterNotifyingHost(PingPongDelayAudioProcessor::delayTimeParam, (float)delayTimeSlider.getValue());
	}
	if (slider == &feedbackSlider) {
		getProcessor()->setParameterNotifyingHost(PingPongDelayAudioProcessor::feedbackParam, (float)feedbackSlider.getValue());
	}
	if (slider == &dryMixSlider) {
		getProcessor()->setParameterNotifyingHost(PingPongDelayAudioProcessor::dryMixParam, (float)dryMixSlider.getValue());
	}
	if (slider == &wetMixSlider) {
		getProcessor()->setParameterNotifyingHost(PingPongDelayAudioProcessor::wetMixParam, (float)wetMixSlider.getValue());
	}
}
